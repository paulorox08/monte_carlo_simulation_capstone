...
OUTPUT_DIR = Path(__file__).parent / "six_figures"
...
def simulate_one_day(n_users=6, minutes_per_day=1440):
    results = []
...
def monte_carlo_simulation(iterations=10000, n_users_range=(6,6)):
    peak_demands = []
...
def plot_analysis(peak_demands):
    ...
    plt.ylabel("Density")
    plt.tight_layout()
    plt.savefig("peak_histogram_test_newer.png")
    plt.show()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig("peak_cdf_test_newer.png")
    plt.show()
...
def plot_stacked_area(df):
    ...
    plt.tight_layout()
    plt.savefig("stacked_area_apps_test_newer.png")
    plt.show()
